...
<head>
    ...
    <style>
        :root {
            ...
            --light-color: #f8f9fa;
            --accent-color: #ffc107;
            --priority-color: #dc3545; /* Red color for priority items */
        }
...
<body>
                ...
                <div>
                    <span class="badge bg-warning text-dark me-2">
                        ...
                        <?php
                            ...
                            $count_normal_query = mysqli_query($con, "SELECT COUNT(*) as total FROM `reqform` WHERE `status`='Pending' AND `date_filed` > '".$three_days_ago->format('Y-m-d')."'");
                    ...
                    <span class="badge bg-danger">
                        Priority: 
                        <?php
                            $count_priority_query = mysqli_query($con, "SELECT COUNT(*) as total FROM `reqform` WHERE `status`='Pending' AND `date_filed` <= '".$three_days_ago->format('Y-m-d')."'");
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            <?php
                                $now = new DateTime();
                                $sql = mysqli_query($con, "SELECT *, DATEDIFF(NOW(), date_filed) as days_waiting FROM `reqform` WHERE `status`='Pending' ORDER BY days_waiting DESC, `controlnumber` ASC");
                                while($fetch = mysqli_fetch_assoc($sql)){
                                    ...
                                    // Determine if it's a priority (more than 3 days)
    ...
    <script>
        ...
        viewModal.addEventListener('show.bs.modal', function (event) {
            ...
            var isPriority = button.getAttribute('data-priority') === 'true' ? true : false;
            ...
            var processBtn = viewModal.querySelector('#process-btn');
            ...
            var modalPDate = viewModal.querySelector('#modal-pdate');
            var modalRespPerson = viewModal.querySelector('#modal-resperson');

            var actionControlNumber = viewModal.querySelector('#action-controlnumber');

            // Check if it's a priority item and update UI accordingly
            if (isPriority) {
